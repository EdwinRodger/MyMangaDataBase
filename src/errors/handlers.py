...
# List of reactions for 405 error
reactions_405 = ["no", "bleh"]
...
def error_405(error):
    url = gif_url(405)
    return render_template("errors/405.html", url=url), 405
...
def err(error):
    ...
    if error == 405:
        url = gif_url(405)
        return render_template("errors/405.html", url=url)
    if error == 500:
        url = gif_url(500)
