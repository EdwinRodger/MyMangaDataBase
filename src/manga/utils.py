...
def save_picture(form_picture):
    ...
    picture_path = os.path.join(
        current_app.root_path, "static/manga_cover/", picture_fn
    )
...
class MangaHistory:
    ...
    def add_chapter(self, manga_name, new_chapter_number):
        ...
        if manga_name in self.history:
            ...
            old_chapter_number = self.history[manga_name][-1][
                0
            ]  # self.history[manga_name][recent entry/chapter of manga_name][chapter_number]
            if old_chapter_number != new_chapter_number:
                self.history[manga_name].append(
                    (new_chapter_number, current_date, current_time)
                )
        else:
