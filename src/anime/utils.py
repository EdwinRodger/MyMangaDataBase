...
def save_picture(form_picture):
    ...
    picture_path = os.path.join(
        current_app.root_path, "static/anime_cover/", picture_fn
    )
...
class AnimeHistory:
    ...
    def add_episode(self, anime_name, new_episode_number):
        ...
        if anime_name in self.history:
            ...
            old_episode_number = self.history[anime_name][-1][
                0
            ]  # self.history[anime_name][recent entry/episode of anime_name][episode_number]
            if old_episode_number != new_episode_number:
                self.history[anime_name].append(
                    (new_episode_number, current_date, current_time)
                )
        else:
            self.history[anime_name] = [
                (new_episode_number, current_date, current_time)
            ]
        self.commit()
